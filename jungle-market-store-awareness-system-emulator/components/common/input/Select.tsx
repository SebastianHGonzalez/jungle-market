...
import { shape, arrayOf, bool, string, func, oneOfType } from "prop-types";
...
function mapValueToOption(value, options, getOptionValue) {
  return Array.isArray(value)
    ? value.map(singleValue =>
        mapSingleValueToOption(singleValue, options, getOptionValue)
      )
    : mapSingleValueToOption(value, options, getOptionValue);
}

function MappingSelect<T>({ value, options, getOptionValue, ...props }: IProps<T>) {
...
function StyledSelect<T>({
  disabled,
  loading,
  theme: { primary, primary75, primary50, primary25 },
  ...props
}: IProps<T>) {
  return (
    <SortingSelect
...
function SortingSelect<T>({ sort, options, ...props }: ISelectProps<T>) {
  const sortedOptions = useMemo(() => (sort ? sortOptions(options) : options), [
    options,
    sort
  ]);

  return <SwitchingSelect {...props} options={sortedOptions} />;
}

const ReadOnlySelectWrapper = styled(Input).attrs({ as: "span" })`
`;

function ReadOnlySelect<T>({ value, getOptionLabel, isLoading, ...props }: ISelectProps<T>) {
  return (
    <InputWrapper>
      <ReadOnlySelectWrapper>
        {[value].flat().map(child => getOptionLabel(child))}
      </ReadOnlySelectWrapper>
      {isLoading && <Loader />}
    </InputWrapper>
  );
...
function SwitchingSelect<T>({ readOnly, creatable, ...props }: IProps<T>) {
